...
def DisenioVigaSimplementeReforzada():
    ...
    if tiene_seccion(ancho, peralte):
        print("Aplicando metodo Iterativo\n")
        uso_metodo_iterativo = True
        area_acero = MetodoIterativo(
            ancho, peralte, recubrimiento, res_concreto, res_acero, momento_ultimo)
    else:
        print("Aplicando metodo de cuantias maximas y minimas\n")
        uso_metodo_iterativo = False
        area_acero, ancho, peralte_efectivo = CuantiasMaximasMinimas(
            res_concreto, res_acero, momento_ultimo)

    numero_varillas, area_acero_real, tipo_varilla = CalcularNumeroVarillas(
        area_acero)

    if uso_metodo_iterativo:
        resultado = VerificarMaximosMinimos(
            area_acero_real, ancho, peralte, recubrimiento, res_concreto, res_acero)
        margen_error = 10
        while resultado == False and margen_error > 0:
            margen_error = margen_error - 0.01
            numero_varillas, area_acero_real, tipo_varilla = CalcularNumeroVarillas(
                area_acero, margen_error)
            resultado = VerificarMaximosMinimos(
                area_acero_real, ancho, peralte, recubrimiento, res_concreto, res_acero)
        if resultado == False:
    ...
    MostrarConfiguracionVarillas(numero_varillas, area_acero_real, ancho, peralte,
                                 peralte_efectivo, recubrimiento, uso_metodo_iterativo, tipo_varilla)
