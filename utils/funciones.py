...
def Cuantia_max(Cuantia_bal):
    ...
    Ro = 0.75 * Cuantia_bal
    print(
        f"se realizara el proceso del calculo para la cuantia maxima con un Ro de: {Ro}")
    return Ro
...
def Cuantia_min(res_concreto, res_acero, verbose=False):
    ...
    if verbose:
        print(
            f"se realizara el proceso del calculo para la cuantia mínima con un Ro de: {Ro}")
    return Ro
...
def beta1_fc(res_concreto):
    """
    Inicializamos el valor de beta1 con cero, para despues, usar la función beta1_fc, cuyo valor se reemplaze 
        a la variable beta1, con le nuevo valor.
    """
    beta1 = 0
...
def valor_incial_para_tanteo(res_concreto, res_acero, momento_ultimo, Ro):
    valor_calculado = (momento_ultimo * 10**5)/(0.9 * res_acero *
                                                Ro * (1-(0.59 * Ro * (res_acero/res_concreto))))
...
def cuantia_balanceada(res_concreto, res_acero, beta1):
    Cuantia_bal = (0.85 * res_concreto * beta1 * 6000) / \
        (res_acero * (6000 + res_acero))
    return Cuantia_bal
...
def CalcularNumeroVarillas(area_acero, margen_error=10):
    ...
    varillas_nominales = {'6mm': 0.28, '8mm': 0.50, '3/8"': 0.71, '12mm': 1.13,
                          '1/2"': 1.29, '5/8"': 1.99, '3/4"': 2.84, '1"': 5.10, '1 3/8"': 10.06}
...
def MostrarConfiguracionVarillas(numero_varillas, area_acero_real, ancho, peralte, peralte_efectivo, recubrimiento, uso_metodo_iterativo, tipo_varilla):
    ...
    if uso_metodo_iterativo:
        peralte_efectivo = peralte - recubrimiento
        print(f"La seccion de la viga es de: ancho = {ancho}, peralte = {
              peralte}, recubrimiento = {recubrimiento}")
        print(f"peralte efectivo = {peralte_efectivo}")
        print(f"El acero real en la viga es de: {
              area_acero_real} y el numero de varillas es de {numero_varillas} de {tipo_varilla}")
    else:
...
if '__main__' == __name__:
    numero_varillas_optimo_funcion, area_acero_real_funcion, tipo_varila_funcion = CalcularNumeroVarillas(
        area_acero=25.5)
    print(f"numero de varillas optimo : {numero_varillas_optimo_funcion}")
    print(f"area de acero real: {area_acero_real_funcion}")
    print(f"tipo de varilla: {tipo_varila_funcion}\n")
